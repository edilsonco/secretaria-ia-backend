/* api/ia.js  –  Secretaria IA  (fuso fixo America/Sao_Paulo) */
...
function parseDateTime(texto) {
  /* Hora obrigatória; Data opcional; “amanhã” = hoje(local)+1 */
  ...
  if (dataM) {
    ...
    // calcula amanhã no fuso local SP
    const now = new Date();
    const localMs = now.getTime() - now.getTimezoneOffset() * 60000;
    const amanha = new Date(localMs + 24 * 60 * 60 * 1000);
    isoDate = amanha.toISOString().slice(0, 10);   // AAAA-MM-DD
  } else {
    isoDate = new Date().toISOString().slice(0, 10); // hoje
  }
  return `${isoDate}T${hh}:${mm}:00-03:00`;          // UTC-3 fixo
}
...
const tituloComp = txt => `Reunião com ${pessoaDoTexto(txt) ?? 'Contato'}`;
